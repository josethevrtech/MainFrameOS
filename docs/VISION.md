...
Make supported Snapdragon computers useful as modern Linux workstations: office work, browsers, coding, communications, media, creative applications and large-screen multitasking. Give applications a consistent place in the desktop regardless of their runtime, while making compatibility limitations visible.
...
1. Preserve reliable boot, graphics, input, networking and recovery on the reference laptop.
...
Target one OmniBook profile, with a Plasma Wayland desktop, working ARM64 Flatpaks, an Android proof of concept, and a small Windows application compatibility record. Publish an image manifest, documented installation/recovery approach and an honest list of remaining limitations.
...
Standalone VR installations, unrelated Snapdragon generations, universal `.exe` compatibility, a custom desktop compositor, a custom app store backend, mandatory gaming components, and NPU-dependent core functionality are not first-release requirements.

For VR, the eventual productivity goal could include large virtual displays and spatial workspaces. First prove boot access, graphics and tracking on a named headset; then select an XR session. A laptop-to-headset streaming workflow is a different deliverable from replacing a headset's operating system.
