...
- Device profile packages: detected board/configuration matching, firmware requirements and hardware settings selected by the common installer.
...
6. Produce a shared installer artifact with versioned platform payloads and checksums; select configuration through hardware detection.
...
Documentation checks and package builds can run without laptop access. Emulated ARM or generic ARM runners can check userspace logic but cannot certify a device's audio, GPU, suspend, display or firmware. Use a dedicated test machine for those checks.
