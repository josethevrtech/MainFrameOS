...
The wireless build input now requires a personally provisioned board database and a provenance JSON using the same required fields as device firmware, with `files` containing exact SHA256 values for `board-2.bin`, `amss.bin`, `m3.bin` and `regdb.bin` from one working firmware set. All four files must be siblings of the supplied board database. The builder checks the initial hardware calibration key and stages the file in the firmware override directory without replacing the packaged database. `wireless-regdb` is also installed; no country override is selected automatically.
...
## Second peripheral test

The next physical test still failed for wireless and showed no desktop audio devices. The saved wireless journal shows a firmware crash with the older packaged runtime after the board database was supplied. The working local installation uses a different runtime. The build now requires and verifies the complete personally provisioned runtime and board set together. This corrects a known mismatch but needs another physical test; the firmware crash is not yet proven resolved.

The audio card and custom UCM profile now load, and saved controls show the headphone and microphone routes. A container probe with hardware writes blocked also discovers and selects HiFi. This does not prove desktop device enumeration or sound playback. No audio fix is claimed from that probe.

The development preview now records audio devices, permissions, PipeWire objects and service logs automatically after login. Reports stay on the USB in `~/.local/state/mainframeos/audio`, use private permissions and are replaced on the next login. Nothing is uploaded. Startup logging follows [WirePlumber's documented debug settings](https://pipewire.pages.freedesktop.org/wireplumber/daemon/logging.html). Wait about one minute on the desktop before shutting down so the next inspection can distinguish missing kernel PCM devices from a desktop audio service failure. Internal speakers remain disabled.
